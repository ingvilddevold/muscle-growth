...
import numpy as np
import ufl
from dolfinx import cpp, geometry, mesh
...
def find_colliding_cells(mesh_big: mesh.Mesh, mesh_small: mesh.Mesh):
    ...
    num_big_cells = (
        mesh_big.topology.index_map(mesh_big.topology.dim).size_local
        + mesh_big.topology.index_map(mesh_big.topology.dim).num_ghosts
    )
    num_small_cells = (
        mesh_small.topology.index_map(mesh_small.topology.dim).size_local
        + mesh_small.topology.index_map(mesh_small.topology.dim).num_ghosts
    )

    bb_tree = geometry.bb_tree(
        mesh_big,
        mesh_big.topology.dim,
        entities=np.arange(num_big_cells, dtype=np.int32),
    )
    big_process = bb_tree.create_global_tree(mesh_big.comm)
    bb_small = geometry.bb_tree(
        mesh_small,
        mesh_small.topology.dim,
        entities=np.arange(num_small_cells, dtype=np.int32),
    )

    process_collisions = geometry.compute_collisions_trees(bb_small, big_process)
    outgoing_edges = set()
    ...
    small_to_big_comm = mesh_small.comm.Create_dist_graph(
        list([mesh_small.comm.rank]),
        [len(np.unique(outgoing_edges))],
        outgoing_edges,
        reorder=False,
    )
    ...
    node_counter = np.zeros(
        mesh_small.geometry.index_map().size_local
        + mesh_small.geometry.index_map().num_ghosts
        + 1,
        dtype=np.int32,
    )
    ...
    for i in range(len(dest)):
        ...
        org_nodes = cpp.mesh.entities_to_geometry(
            mesh_small._cpp_object,
            mesh_small.topology.dim,
            sending_cells[process_offsets[i] : process_offsets[i + 1]],
            False,
        ).reshape(-1)
    ...
    for i in range(len(source)):
        local_meshes.append(
            mesh.create_mesh(
                MPI.COMM_SELF,
                cells=recv_top[recv_top_offsets[i] : recv_top_offsets[i + 1]].reshape(
                    -1, num_vertices_per_cell_small
                ),
                x=recv_geom[recv_geom_offsets[i] : recv_geom_offsets[i + 1]].reshape(
                    -1, mesh_small.geometry.dim
                ),
    ...
    def extract_cell_geometry(input_mesh, cell: int):
        mesh_nodes = cpp.mesh.entities_to_geometry(
            input_mesh._cpp_object,
            input_mesh.topology.dim,
    ...
    for i in range(len(source)):
        ...
        local_tree = geometry.bb_tree(local_meshes[i], local_meshes[i].topology.dim)
        cell_cell_collisions = geometry.compute_collisions_trees(local_tree, bb_tree)
    ...
    sorted_unique_small_cells = np.unique(small_mesh_colliding_cells).astype(
        dtype=np.int32
    )
...
def create_midpoint_plane(
    mesh_3d: mesh.Mesh, padding: float = 1.2, resolution: int = 20
):
    """
    ...
    coordinate_element = ufl.Mesh(
        basix.ufl.element(
            org_coordinate_element.family_name,
            rect_mesh_2d.basix_cell(),
            org_coordinate_element.degree,
            shape=(3,),
        )
    )
...
def create_midpoint_box(
    mesh_3d: mesh.Mesh,
    thickness: float,
    padding: float = 1.2,
    resolution_xyz=(5, 50, 50),
):
    """
    ...
    box_local = mesh.create_box(
        MPI.COMM_WORLD, [p0, p1], resolution_xyz, mesh.CellType.tetrahedron
    )
    ...
    bottom_facets = mesh.locate_entities_boundary(
        box_local, box_local.topology.dim - 1, bottom_face_local
    )
    bottom_values = np.full_like(bottom_facets, 1, dtype=np.int32)
    sorted_facets = np.argsort(bottom_facets)
    box_facet_tags = mesh.meshtags(
        box_local,
        box_local.topology.dim - 1,
        bottom_facets[sorted_facets],
        bottom_values[sorted_facets],
    )
