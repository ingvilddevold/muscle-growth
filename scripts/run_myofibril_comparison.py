"""
This script runs simulations for three different exercise protocols using the
modified config file with manually fitted k1 (kM). It saves the full ODE
solutions to CSV files and is intended to be called from a Snakemake rule.
"""
...
print("Starting myofibril comparison simulations...")
...
for i, protocol_name in enumerate(protocol_names):
    ...
    # Instantiate the model with the specified config file.
    model = ExerciseModel(
        protocol,
        str(config_file),
    )
...
print("All myofibril comparison simulations complete.")
