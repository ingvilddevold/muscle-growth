...
def process_ode_states_sequentially(
    ode_bp_files: list[Path], labels: list[str], ode_states: list[str]
) -> pd.DataFrame:
    """
    ...
    for bp_file, label in zip(ode_bp_files, labels):
        try:
            ...
            ode_funcs = {
                state: dolfinx.fem.Function(V_ode, name=state) for state in ode_states
            }
            ...
            print(
                f"    Reading {len(times)} time steps for {len(ode_states)} states..."
            )
...
def aggregate_data(
    ode_bp_files: Annotated[
        str, typer.Option(help="Comma-separated paths to ode_spatial_history.bp files.")
    ],
    variation_labels: Annotated[
        str, typer.Option(help="Comma-separated string of variation labels.")
    ],
    output_csv: Annotated[
        Path, typer.Option(help="Path to save the aggregated data CSV file.")
    ],
):
    """
    ...
    if not (len(bp_files) == len(labels)):
        print(
            f"Error: Mismatch in number of files ({len(bp_files)}) and labels ({len(labels)})."
        )
    ...
    ode_agg_df = process_ode_states_sequentially(
        bp_files, labels, ode_states_to_aggregate
    )
...
def plot_data(
    ...
    ensemble_stats = (
        variations_df.groupby(["state", "Time (weeks)"])
        .agg(
    ...
    for i, (state_name, ax) in enumerate(plot_map.items()):
        ...
        if i == 0:
            ...
            legend_elements = [
                Line2D(
                    [0], [0], color=baseline_color, lw=1.0, ls="--", label="Uniform"
                ),
